...
export function WorshipCard({ dayLabel, time, playlistUrl, sheetUrl, sermonPassage, sermonTopic, sheets = [], sheetPageHref, noteKey, onEmptyLink, children, noteLabel, notePlaceholder = defaultNotePlaceholder }: WorshipCardProps) {
  ...
  return <ProgramCard eyebrow={dayLabel} title="저녁예배" icon={<ProgramIcon kind="worship" />} time={time}>
    <dl className="sermon-meta">
      <div className="sermon-meta__praise"><dt>찬양</dt><dd><div className="button-row sermon-meta__actions">
        <button type="button" className="button button--secondary button--small" onClick={() => openLink(playlistUrl)} aria-label={`${dayLabel} 찬양 플레이리스트 열기`}>찬양 플레이리스트</button>
      ...
      <div><dt>설교 말씀 본문</dt><dd>{sermonPassage || "추후 업데이트 예정"}</dd></div>
